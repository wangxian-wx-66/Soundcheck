...
/** 完整缓存键：序列化全部入参——v2.20 修复：原实现解构 {query,count,filter,sort}，
 *  questionAnswers 传的 {questionUrl,limit}、hotList 传的 {limit} 被整体丢弃，
 *  导致所有问题的 question_answers 共用一个键（跨问题错误命中：A 问题的标杆回答喂给 B 问题） */
export function searchCacheKey(apiType, params = {}) {
  return createHash('sha256')
    .update(JSON.stringify({ apiType, ...params, apiVersion: API_VERSION }))
    .digest('hex');
}
