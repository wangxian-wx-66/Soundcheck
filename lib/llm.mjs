...
function mockChat(script) {
  const steps = [...(script || defaultMockScript())];
  return async ({ messages, tools, jsonMode }) => {
    ...
    // 工具轮：有 steps 且对话中存在工具结果（或尚无任何 assistant 消息）时取下一步
    if (tools?.length && steps.length) {
    ...
    // 最终报告（含 chatJson 重问轮）
    return { role: 'assistant', content: typeof steps[0]?.content === 'string' ? steps.shift().content : JSON.stringify(mockReport()) };
  };
}
