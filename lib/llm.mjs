...
function mockReport(messages) {
  const report = {
    ...
    coverage: {
      ...
      argument_map: [
        // 含典型 AI 腔记号（层 3 humanize 清洗的端到端测试锚点）
        { argument: '先补数学基础再入门（线性代数/概率论（矩阵论））', status: 'covered', evidence_level: 'L1', source_urls: ['https://www.zhihu.com/answer/53910077'] },
    ...
    increment: {
      covered: ['先补数学基础再入门（线性代数/概率论（矩阵论））', '项目驱动边做边学'],
      unique: ['课程+教材组合路径'],
      blanks: ['入门阶段的常见误区'],
      amplify: '建议强化第 3 条论点，按[选教材→跟课程→做项目]顺序展开，补充具体学习周期与资源清单。',
    },
    controversy: {
      objections: [
        { objection: '数学基础不用先学全', response: '承认按需补的合理性，拆成[最小集合→项目倒逼→按需深挖]回应，强调最小集合', evidence: 'real', source: '数学基础真的重要，别跳过' },
