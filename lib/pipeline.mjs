  ...
  async function runCynicReview(run, report, evidence, signal) {
    ...
    try {
      ...
      const nitpicks = array(parsed.nitpicks).slice(0, 3).map((n) => ({
        objection: String(n.objection || '').slice(0, 200),
        why: String(n.why || '').slice(0, 200),
      ...
      const disagreements = array(parsed.disagreements).slice(0, 3).map((d) => ({
        point: String(d.point || '').slice(0, 80),
        chief: String(d.chief || '').slice(0, 200),
        cynic: String(d.cynic || '').slice(0, 200),
      }));
      const result = {
        nitpicks,
