...
export function humanize(text) {
  return String(text)
    ...
    .replace(/\s*\b(evidence_level|own_rating|question_title|argument_map|increment|coverage|controversy|radar|rating|unique)\b[\s::]*(?=[\u4e00-\u9fff])/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}
...
- 禁止把内部字段名写进给用户看的文本（如 evidence_level、rating、unique、JSON 等英文标识符）——用中文自然表述对应概念。
