 ...
 *  ④内部字段名泄漏（evidence_level 给到 L1）→ 剥字段名（「evidence_level 给到 L1」→「给到 L1」）
 *  ⑤多余空白归一 */
...
export function humanize(text) {
  return String(text)
    ...
    // ④ 内部字段名泄漏：仅当键名后紧跟中文时剥除（真 key 实测「evidence_level 给到 L1」）；
    //    环视守卫防止误伤正常英文句子（The coverage of this answer 的 coverage 不动）
    .replace(/\s*\b(evidence_level|own_rating|question_title|argument_map|increment|coverage|controversy|radar|rating|unique|CommentInfoList|question_answers|zhihu_search|global_search|hot_list)\b[\s::]*(?=[\u4e00-\u9fff])/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}
...
- 禁止把内部字段名或接口名写进给用户看的文本（如 evidence_level、rating、unique、CommentInfoList、question_answers、JSON 等英文标识符）——用中文自然表述对应概念（如 CommentInfoList 说「真实评论」，question_answers 说「默认排序」）。
- 枚举改顿号或「和」，限定语拆成独立短句。口语但不失专业，就像知乎高赞回答里那种自然的行文。
- JSON 结构、字段名、证据等级标注不受本段影响（指的是 JSON 键，不是值里的文本）。
...
- 禁止把内部字段名和记号写进给用户看的文本（如 evidence_level、rating、own_rating、L1、unique、JSON 等英文标识符）——想表达证据等级就说「有真实评论撑腰」，想表达评级就说「我给 B」。
- JSON 结构、字段名、evidence 标注不受本段影响（指的是 JSON 键，不是值里的文本）。
...
${run.questionUrlHit ? `【目标问题 URL】${run.questionUrlHit}\n` : ''}请按系统指令执行检索与分析${
  ...
  function collectEvidence(evidence, items) {
    for (const item of items.slice(0, 5)) {
      evidence.chips.push({
        type: 'search',
        label: `${item.votes} 赞 · ${item.authority ? `权威 L${String(item.authority).replace(/^L/i, '')}` : '权威未知'} · ${String(item.author || '').slice(0, 12)}`,
      });
    }
    ...
    return {
      ...
      controversy: {
        objections: array(report?.controversy?.objections).slice(0, 5).map((o) => ({
          ...
          source: humanize(String(o.source || '')).slice(0, 200),
        })),
      },
      radar,
      meta: {
        confidence: ['high', 'medium', 'low'].includes(report?.meta?.confidence) ? report.meta.confidence : 'medium',
        note: humanize(String(report?.meta?.note || '判断基于检索摘要（非全文），请点击原文链接对照。')).slice(0, 1200),
      },
      trace: {
        mode: run.mode,
